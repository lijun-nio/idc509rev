...
//!
//! ---
//! Vendored verbatim from the `c509` certificate reference implementation
//! (`c509_demo_impl/src/lcbor.rs`, cose-wg/CBOR-certificates @ bd0da3ef),
//! licensed BSD-3-Clause (© the C509 authors: John Preuß Mattsson, Joel
//! Höglund). Copied — rather than depended on by path — so that `c509rev`
//! builds standalone in any clone of this repository, without needing the
//! sibling cert-implementation checkout. `lcbor` is deterministic RFC 8949
//! CBOR and draft-stable; keep it byte-for-byte in sync with the upstream
